...
def test_list_exam_styles_has_four_known_styles(catalog):
    styles = list_exam_styles(catalog)
    assert set(styles) >= {"default", "dzexams", "bem", "bac"}
    assert "name_ar" in styles["dzexams"]
    assert styles["dzexams"]["uses_situations"] is True
    assert styles["bem"]["template_html"] == "exam_dzexams.html"


def test_resolve_exam_style_explicit(catalog):
    assert resolve_exam_style(catalog, "dzexams", stage="middle") == "dzexams"
    assert resolve_exam_style(catalog, "bem", stage="middle") == "bem"
    assert resolve_exam_style(catalog, "bac", stage="secondary") == "bac"


def test_resolve_exam_style_unknown_falls_back(catalog):
    # نمط غير معروف → نرجع للاستدلال
    assert resolve_exam_style(catalog, "xyz", stage="middle") == "default"


def test_resolve_exam_style_inferred_from_exam_type(catalog):
    assert (
        resolve_exam_style(
            catalog, None, stage="secondary", exam_type="بكالوريا تجريبية",
        )
        == "bac"
    )
    assert (
        resolve_exam_style(
            catalog, None, stage="middle",
            exam_type="شهادة التعليم المتوسط تجريبية",
        )
        == "bem"
    )
    assert resolve_exam_style(catalog, None, stage="middle", exam_type="فرض محروس") == "default"


def test_get_structure_with_dzexams_style_middle(catalog):
    s = get_exam_structure(catalog, "middle", "اختبار فصلي", style="dzexams")
    assert s is not None
    parts = s["parts"]
    assert len(parts) == 3
    assert all("الوضعية" in p["name"] for p in parts)
    assert sum(p["points"] for p in parts) == 20


def test_get_structure_with_bem_style(catalog):
    s = get_exam_structure(catalog, "middle", "اختبار", style="bem")
    assert s is not None
    parts = s["parts"]
    # 3 تمارين + وضعية إدماجية = 4 أجزاء
    assert len(parts) == 4
    assert sum(p["points"] for p in parts) == 20
    assert any("وضعية" in p["name"] or "إدماج" in p["name"] for p in parts)


def test_get_structure_with_bac_style(catalog):
    s = get_exam_structure(catalog, "secondary", "اختبار", style="bac")
    assert s is not None
    # موضوعان مخيران
    parts = s["parts"]
    assert len(parts) == 2
    assert all(p["points"] == 20 for p in parts)


def test_get_structure_with_dzexams_style_secondary(catalog):
    s = get_exam_structure(catalog, "secondary", "اختبار فصلي", style="dzexams")
    assert s is not None
    assert len(s["parts"]) == 3
    assert sum(p["points"] for p in s["parts"]) == 20


# ------------------ distribute_points_for_situations ------------------


def test_distribute_situations_default_index(catalog):
    parts = distribute_points_for_situations(catalog, "middle", "dzexams", rotation_index=0)
    assert len(parts) == 3
    assert sum(p["points"] for p in parts) == 20


def test_distribute_situations_rotation_changes_distribution(catalog):
    p0 = distribute_points_for_situations(catalog, "middle", "dzexams", rotation_index=0)
    found_different = False
    for r in range(1, 6):
        pi = distribute_points_for_situations(catalog, "middle", "dzexams", rotation_index=r)
        if [p["points"] for p in pi] != [p["points"] for p in p0]:
            found_different = True
            break
    assert found_different, "rotation should select alternative distributions"


def test_distribute_situations_unknown_stage_returns_empty(catalog):
    assert distribute_points_for_situations(catalog, None, "dzexams") == []
    assert distribute_points_for_situations(catalog, "middle", "unknown_style") == []
