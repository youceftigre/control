...
def validate_request(body: dict) -> tuple:
    ...
    valid_types = QUESTION_TYPES_BY_SUBJECT.get(subject, [])
    invalid_types = [t for t in types if t not in valid_types]
    if invalid_types:
        return False, f"أنواع أسئلة غير صالحة للمادة {subject}: {', '.join(invalid_types)}. الأنواع المقبولة: {', '.join(valid_types)}"
    
    # التحقق من وجود وضعية إدماجية
    has_integration = any('إدماج' in str(t) or 'intégration' in str(t).lower() or 'integration' in str(t).lower() for t in types)
    if not has_integration:
        return False, "يجب تضمين الوضعية الإدماجية في الاختبار"
...
def build_prompt(body: dict) -> str:
    """بناء التعليمات المحسّنة حسب المادة"""
...
def validate_generated_content(content: str, expected_subject: str) -> tuple:
    ...
    # التحقق من وجود الوضعية الإدماجية
    if 'إدماج' not in content and 'intégration' not in content.lower() and 'integration' not in content.lower():
        errors.append("❌ الوضعية الإدماجية مفقودة")
...
def generate():
    ...
    try:
        ...
        # بناء التعليمات المخصصة للمادة
        prompt = build_prompt(body)
        
        # استدعاء API مع إعدادات محسّنة
        chat_completion = client.chat.completions.create(
            messages=[
                {
                    "role": "system", 
                    "content": f"أنت مفتش تعليم جزائري خبير في مادة {subject} فقط. تولد اختبارات دقيقة ومنظمة تتوافق مع المنهاج الرسمي الجزائري. ممنوع ذكر أي مادة أخرى غير {subject}."
                },
                {
                    "role": "user", 
                    "content": prompt
            ...
            temperature=0.2,  # درجة حرارة منخفضة جداً للدقة القصوى
            max_tokens=8000,
            top_p=0.9
        )
