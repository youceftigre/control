    ...
    def load_data(self):
        ...
        try:
            if os.path.exists('data/questions_full_bank.json'):
                with open('data/questions_full_bank.json', 'r', encoding='utf-8') as f:                    self.question_bank = json.load(f)
            ...
            if os.path.exists('data/subjects_config.json'):
                ...
                logger.info(f"✅ تم تحميل إعدادات {len(self.subjects_config)} مادة")
...
class ExamGenerator:
    ...
    def generate_questions(self, subject, grade, semester, exam_type, topic, difficulty, count=5):
        ...
        if not self.client:
            return {'error': 'خدمة الذكاء الاصطناعي غير متاحة حالياً'}
...
الصيغة: JSON بالضبط دون أي شرح إضافي
...
def generate_exam():
    ...
    try:
        ...
        result = exam_generator.generate_questions(
            data.get('subject'), data.get('grade'), data.get('semester'),
            data.get('examType', 'اختبار فصلي'), data.get('topic', ''),
            data.get('difficulty', 'متوسط'), 5
        )
...
def filter_bank():
    ...
    try:
        ...
        logger.error(f"❌ خطأ في التصفية: {e}")        return jsonify({'error': str(e)}), 500
...
def get_stats():
    ...
    try:
        stats = {
            'total_questions': 0, 'by_subject': {}, 'by_level': {},
            'by_type': {}, 'by_difficulty': {1: 0, 2: 0, 3: 0}
        }
        for subject, grades in data_manager.question_bank.items():
            stats['by_subject'][subject] = 0
            for grade, chapters in grades.items():
                if 'ابتدائي' in grade: level = 'ابتدائي'
                elif 'متوسط' in grade: level = 'متوسط'
                else: level = 'ثانوي'
...
@app.errorhandler(429)def ratelimit_handler(e): return jsonify({'error': 'Too many requests'}), 429
